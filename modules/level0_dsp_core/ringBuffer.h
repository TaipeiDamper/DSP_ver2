...
public:
    ...
    void reset() {
        std::fill(buffer.begin(), buffer.end(),
                            0.0f); // when reset, fill buffer with all 0
        writeIdx = 0;    // reset write ptr (calculate read ptr in normal cases)
    };

    void setBufferLength(int bufferLength_) {
        bufferLength =
                std::max(1, bufferLength_) + 4; // 4 for extra margin for interpolation
    ...
    void writeSample(float sample_) {
        ...
        writeIdx = wrapIdx(writeIdx + 1); // update write index嚙稽circular嚙稷
    }

    // [default]  read sample before write
    float readSample(int indexOffset_) {
        return buffer[wrapIdx(writeIdx - indexOffset_)];
    }

    // readSample after "writeSample" (since the index had changed)
    float readTap(int indexOffset_) {
        return buffer[wrapIdx(writeIdx - 1 - indexOffset_)];
    }
    ...
    float readSample(float indexOffsetFrac_) {
        ...
        int readIdx = wrapIdx(
                static_cast<int>(floor(readPos))); // wrap readIdx and get int part
...
protected:
    int wrapIdx(int idx_) { return (idx_ + bufferLength) % bufferLength; }
};
