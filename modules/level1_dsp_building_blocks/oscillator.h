...
/* 2026 / 01 / 07
now it support getVal(offset) + phaseUpdate() for multichannel-phaseOffset
tuning
*/
...
public:
    ...
    */
    ...
    void setPhase(float phase_) { phase = phase_; }
    ...
    */
    void phaseUpdate() { phase = wrapPhase(phase + phaseDelta); }
...
protected:
    ...
    */
    virtual float evalPhase(float phase_) { return phase_; }
...
public:
    float processSample() override {
        float fmHz = modulatorOsc.processSample() *
                                  depth; // modulated frequency offset in Hz
