...
public:
  ...
  void setDelayInSec(float delayInSec_) {
    ...
    if (delayInSec > maxDelayInSec) {
      delayInSec = maxDelayInSec;
    }
...
public:
  ...
  void setDelayGain(float delayGain_) { delayGain = delayGain_; }
  void setDelayInSec(float delayInSec_) { delay.setDelayInSec(delayInSec_); }
  void setMaxDelayInSec(float maxDelayInSec_) {
...
public:
  ...
  virtual float processSample(float input_) override {
    float bufferOutput = delay.processSample(
        sampleReadyForBuffer); // take in last output to process
    // y[n] = x[n] + g*y[n-m]
    float output = input_ + delayGain * bufferOutput;
