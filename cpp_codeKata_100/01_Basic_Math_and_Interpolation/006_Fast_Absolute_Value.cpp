...
void runTests()
{
    ...
    if (isClose(fastAbs(1.5f), 1.5f))
    {
        std::cout << "PASS\n";
        passed++;
    }
    else
        std::cout << "FAIL (結果: " << fastAbs(1.5f) << ")\n";

    std::cout << "[測試 2] 負數 -2.7 -> ";
    if (isClose(fastAbs(-2.7f), 2.7f))
    {
        std::cout << "PASS\n";
        passed++;
    }
    else
        std::cout << "FAIL (結果: " << fastAbs(-2.7f) << ")\n";

    std::cout << "[測試 3] 零 -> ";
    if (isClose(fastAbs(0.0f), 0.0f))
    {
        std::cout << "PASS\n";
        passed++;
    }
    else
        std::cout << "FAIL (結果: " << fastAbs(0.0f) << ")\n";

    std::cout << "--------------------------------------\n";
    if (passed == 3)
        std::cout << "🎉 恭喜！KATA 006 挑戰成功！(這是面試經典題喔)\n";
    else
        std::cout << "❌ 還有錯誤，請繼續除錯。\n";
}
